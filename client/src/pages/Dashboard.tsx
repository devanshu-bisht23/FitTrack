...
const Dashboard = () => {
  ...
  const {user, allActivityLogs, allFoodLogs, allStepLogs} = useAppContext()
  ...
  const stepCalories = allStepLogs
    .filter((s)=> s.date === today)
    .reduce((sum, s)=> sum + s.calories, 0)

  const totalBurned: number =
    todayActivities.reduce((sum, item)=> sum + (item.calories || 0), 0)
    + stepCalories

  const adjustedDailyLimit = DAILY_CALORIE_LIMIT + totalBurned;

  const remainingCalories = adjustedDailyLimit - totalCalories;
        ...
        <Card className="shadow-lg col-span-2">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="text-right">
              ...
              <p className="text-2xl font-bold text-slate-800 dark:text-white">{adjustedDailyLimit}</p>
            </div>
          </div>
          <ProgressBar value={totalCalories} max={adjustedDailyLimit}/>
          ...
          <div className="mt-4 flex justify-between items-center">
            ...
            <span className="text-sm text-slate-400">{Math.round((totalCalories / adjustedDailyLimit) * 100)}%</span>
          </div>
